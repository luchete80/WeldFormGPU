...
inline Particle::Particle(int Tag, Vector const & x0, Vector const & v0, double Mass0, double Density0, double h0,bool Fixed)
{
	ct = 0;
	a = 0.0;
    x = x0;
...
// inline void Particle::Move(double dt, Vector Domainsize, Vector domainmax, Vector domainmin, size_t Scheme, Mat3_t I)
// {
	// if (Scheme == 0)
		// Move_MVerlet(I, dt);
	// else if (Scheme == 1)
...
// {
	...
	// Vector du = dt*(v+VXSPH) + 0.5*dt*dt*a;
	// Displacement += du;
	// x += du;
...
// inline void Particle::translate(double dt, Vector Domainsize, Vector domainmax, Vector domainmin)
// {
