...
//				if (P1->IsFree) vab.x = P1->v(0) + P2->vb(0); else vab.x = -P1->vb(0) - P2->v(0);
			}
		}
		...
		float3 temp = make_float3(0.0);
		double temp1 = 0.0;

		if (GradientType == 0)
			temp =  dot(1.0/(di*di)* Sigmai + 1.0/(dj*dj)*Sigmaj + PIij + TIij, (GK*xij) ) ;
			//Mult( GK*xij , ( 1.0/(di*di)*Sigmai + 1.0/(dj*dj)*Sigmaj + PIij + TIij ) , temp);
		else
			temp = dot( 1.0/(di*dj)*(Sigmai + Sigmaj)           + PIij + TIij , GK*xij );
			// Mult( GK*xij , ( 1.0/(di*dj)*(Sigmai + Sigmaj)           + PIij + TIij ) , temp);

		if (Dimension == 2) temp.z = 0.0;
